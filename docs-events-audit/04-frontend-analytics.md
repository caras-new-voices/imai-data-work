...
`da98fcf2` (refs re-verified; signup-flow change noted below). GTM container
internals pending access ([Q15](./OPEN_QUESTIONS.md)); see
[05-gtm-containers.md](./05-gtm-containers.md).
...
> **Funnel change 2026-07-21 (`2d8bfcd9`):** `skipOnboardingLabels` now includes
> `'imai'` (registration.component.ts:191, google-sso.component.ts:40) — ALL
> IMAI signups route straight to `/trial`, skipping `/get-started` onboarding —
> and Gmail signups via Google are allowed again (backend gate commented out,
> users.controller.ts:422-438). Expect `successful_onboarding_completed` (GTM),
> the onboarding-path `trial_success` (GA4) and `finished_onboarding` (webhook)
> to collapse in volume from that date, and the email-domain mix to shift.
...
|---|---|---|---|
| `successful_signup` | registration.component.ts:106 (google), :436 (email) | `method`, `email` | ⚠ raw email into dataLayer (Enhanced Conversions per code comment) |
...
| `successful_onboarding_completed` | onboarding.component.ts:412 | `user_id` | ⚠ Since 2026-07-21 IMAI signups skip onboarding — volume collapses. |
...
| `google_signup` / `signup` | registration.component.ts:105 / :435 | registration |
| `email_verified` | registration.component.ts:463 | registration |

Plus the **server-side** GA4 Measurement Protocol `purchase`
(backend, payments.service.ts:1789-1810 — see 03-emitters.md §4, Q11;
still ungated with hardcoded ids/secret at HEAD).
...
|---|---|---|
| Trial-friction events | `POST /data/pipedrive/trial-event` (trial-gating.service.ts:53) | See 02-event-dictionary.md §5 and 03-emitters.md §2 — the client picks whitelisted server event names (`trial_upgrade_<placement>`, `trial_unlock_modal_opened`, `trial_book_call_*`, `trial_sample_report_viewed`). Backend endpoint now at users.controller.ts:2367. |
...
Note: since 2026-07-21 (`d0d6324f`) the JWT interceptor no longer attaches
`X-White-Label` (or Authorization) to external hosts (GCF uploadFile / GCS) —
CORS preflight fix, no analytics impact (jwt.interceptor.ts:32).
...
| Onboarding done | GTM `successful_onboarding_completed` · GA4 `trial_success` (also fired here!) | webhook `finished_onboarding` | GA4 `trial_success` fires on BOTH credit-card and onboarding paths — inflates trial counts in GA4. ⚠ Whole row collapses in volume after 2026-07-21 (IMAI signups skip onboarding). |
