...
| Q24 | What emits the Title-Case Mixpanel namespace (`Platform Usage` 1.6M/30d, `Influencer Report`, `Login`, `Campaign Media Scraped`, …)? NO emitter exists in the codebase — a second live pipeline. | It's the HIGHEST-volume tracking on the platform and completely undocumented; several names duplicate snake_case events (double-count hazard). | Likely the external webhook pipeline or a legacy direct integration; Q1/Q2 access will reveal it. Check Mixpanel project's data sources / service accounts. | OPEN |
...
| Q20 | Which Mixpanel reports/boards does the team actually use (health score is replicated in-code at usage-board.service.ts:1909 — what else)? | Tells us which events MUST stay stable through any taxonomy cleanup. | Ask Mixpanel users (product/CS); Mixpanel usage report. | OPEN |
